...
def get_loss_mapping(config, image, rendered_depth, viewpoint, opacity):
    ...
    l1_rgb = (1.0 - config['opt_params']['lambda_dssim']) * l1_loss(image, gt_image) + \
              config['opt_params']['lambda_dssim']* (1.0 - ssim(image, gt_image))

    l1_depth = l1dep_loss(rendered_depth * depth_pixel_mask, gt_depth * depth_pixel_mask)
    
    return l1_rgb + config['opt_params']['lambda_dssim']* l1_depth
